...
- **[2026-08-05] [Process] Declare DSL-Inferred Behavior in Every Consumer** — When a DSL feeds multiple consumers (codegen, editor), behavior it does not declare is inferred differently by each — declare the intent in the DSL and validate it through a shared entry point every consumer calls. Seven divergence gaps in one session (chip enumeration/filter, join-note panes, text and chip refs, row taps, list item titles, CRUD display fields) were closed this way with build-time validation shared by the Flutter emitter and the editor.
