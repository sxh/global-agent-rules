...
- **[2026-08-01] [Process] Verify Block Structure After String-Based Block Edits** — String-based edits near block boundaries can corrupt structure in two ways: appending can duplicate block closers (orphaning later code with broken scope), and replacing a block can silently truncate it when the replacement does not reproduce the full original (dropped bodies, constructor params, fields, or braces). After any edit that touches a block boundary, read the edited region immediately and verify structural integrity before running tests — syntax validity alone does not guarantee correct structure. In one session, seven separate compile failures were caused by incomplete edit replacements that deleted function bodies; the truncation was only discovered when compilation failed minutes later.
