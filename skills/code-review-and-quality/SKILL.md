...
Does the change work for keyboard, screen-reader, and assistive-technology users? This axis is not optional for UI changes — focus management and ARIA break in ways unit tests often cannot see (see Test Quality below for the environment-limitation caveat).

- Are interactive elements keyboard-reachable and is focus managed correctly (focus enters a dialog/modal, traps inside it, and restores to the trigger on close)?
- Is every interactive element reachable via Tab/Shift+Tab and operable via keyboard (Enter/Space), not just mouse?
- Does Tab/Shift+Tab cycling stay inside open dialogs/modals — can focus escape the trapped region (e.g., a focusable backdrop outside the trap)?
- Are ARIA attributes correct and unique: `aria-label`/`aria-labelledby` referencing existing, per-instance-unique ids (`useId()`, never hardcoded ids that collide across instances)?
- Is `aria-modal`/`role="dialog"` present on modal containers, and are decorative icons hidden from AT (`aria-hidden`)?
- Are buttons with only icons given both `aria-label` and `title`?
- Do inputs/textarea have accessible names (label or aria-label), not just placeholders?
- Is focus restoration handled on unmount-while-open, not only on the close transition?
- Are layout/typography properties in CSS, not inline styles (except genuinely dynamic values)?
- Do buttons have explicit `type` attributes so they don't default to submit?

### 7. Test Quality

Do the tests actually protect the behavior, or just pass? A green suite is not evidence of correctness if the tests are tautological, leaky, or blind to their environment.
...
### Accessibility
- [ ] Keyboard-reachable and operable (Tab/Shift+Tab, Enter/Space)
- [ ] Focus managed: enters, traps inside, restores on close (incl. unmount-while-open)
- [ ] ARIA attributes correct and per-instance unique (no hardcoded ids)
- [ ] Icon-only buttons have aria-label + title
- [ ] Inputs have accessible names (not just placeholders)
- [ ] Layout/typography in CSS, not inline styles
...
| "The tests pass, so it's good" | Tests are necessary but not sufficient. They don't catch architecture problems, security issues, or readability concerns — and they can be false-green: passing against code that should fail (tautological assertions, jsdom blind spots). Check the tests themselves, not just their green status. |
| "It's not accessible but we'll fix it later" | Focus traps, ARIA ids, and keyboard paths don't fix themselves. A dialog with an escape-able focus trap or hardcoded duplicate ids is broken for keyboard/AT users at merge time — review it on this change, not later. |
