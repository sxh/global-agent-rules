...
- **Profile Before Optimizing**: When targeting performance, measure first to identify the actual bottleneck. No optimization code may be written before the bottleneck is confirmed by empirical data (profiler, timing instrumentation, or benchmark). An optimization that does not change the measured bottleneck is waste.
...
- **Reference, Don't Reimplement**: When introducing logic that parallels an existing abstraction (word boundaries, regex compilation, URL building), read the existing implementation first and verify the behaviour matches. An incorrect assumption about how existing code works produces incorrect new code. This applies when a lint rule pushes a fix too: before choosing the first compliant-looking option, check the sibling/comparative implementation for the accepted pattern — a lint-driven `<button>` wrapper produced invalid nested-button HTML where a `role="presentation"` wrapper already existed in the sibling modal.
...
- **Test the Wire, Not the Wrapper** — Every distinct URL, request body, or serialized payload that crosses a system boundary must have its own test containing the literal expected string. Testing a shared URL-building helper in isolation and assuming all derived URLs are correct is a tautology, not verification. The literal must come from a **real capture of the live system**, never a guess at the format. When adding a protocol consumer: (1) run the real system once and copy its actual output verbatim into the fixture; (2) confirm the fixture **fails** against the assumed-format code before fixing — red against reality, not merely red against the fixture; (3) note the capture origin in a comment. A fabricated literal validates the code against its own assumption and stays green forever while the real flow is broken — a `flutter run --machine` fixture guessed `{"event":"app.start",...}` while the daemon emits a bracket-wrapped array, so every test passed through the entire B133–B141 rework while no launch could succeed.
